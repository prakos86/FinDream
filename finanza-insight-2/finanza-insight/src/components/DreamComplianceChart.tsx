...
export const DreamComplianceChart: React.FC<DreamComplianceChartProps> = ({
          ...
          >
            ...
            <div className="space-y-1.5 text-left">
              ...
              <input
                type="text"
                required
                placeholder="Ej: Comprar mi Carro, Enganche de Casa, Maestría..."
                value={newDreamName}
  ...
  return (
    <div className="space-y-5">
                ...
                <AnimatePresence>
                  {editingSuenoId === s.id && (
                    <motion.div
                      ...
                      className="overflow-hidden"
                    >
                      <div className="mt-3 pt-3 border-t border-slate-200/20 space-y-3" onClick={(e) => e.stopPropagation()}>
                        <div className="space-y-1">
                          <label className={`text-[10px] font-black uppercase tracking-wide ${isSelected ? 'text-indigo-200' : 'text-slate-500'}`}>Nombre del sueño</label>
                          <input
                            type="text"
                            value={s.nombre}
      ...
      {/* --- NUEVO SUEÑO SECTION --- */}
      <div className="bg-white rounded-[25px] p-5 shadow-[0_4px_16px_rgba(0,0,0,0.02)] border border-gray-100 flex flex-col justify-center">
        <div className="flex justify-between items-center cursor-pointer" onClick={() => setShowAddForm(!showAddForm)}>
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-indigo-50 text-indigo-700 rounded-xl flex-shrink-0 animate-pulse border border-indigo-100/50">
               <Sparkles className="w-5 h-5" />
            </div>
            <div>
              <span className="text-[10px] font-black uppercase tracking-widest text-indigo-700 block">Nuevo Sueño</span>
              <h3 className="text-[13px] font-black text-slate-900 mt-0.5">Agregar a mi portafolio</h3>
            </div>
          </div>
          <button
            type="button"
            className={`p-2 rounded-xl border-2 transition-all cursor-pointer flex items-center justify-center ${
              showAddForm ? 'bg-rose-50 border-rose-200 text-rose-600' : 'bg-indigo-50 border-indigo-200 text-indigo-700'
            }`}
          >
        ...
        <AnimatePresence>
          {showAddForm && (
            ...
            >
               <div className="space-y-1.5">
                <label className="text-xs font-black text-slate-950 block">¿Qué sueñas lograr?</label>
                <input
                  type="text"
                  required
                  placeholder="Ej: Viaje a París, Postgrado, Pieza de Arte..."
                  value={newDreamName}
                  onChange={(e) => setNewDreamName(e.target.value)}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-black text-slate-950 block">Ahorro Mensual Planificado</label>
                  <input
                    type="number"
                    required
                    min={1}
